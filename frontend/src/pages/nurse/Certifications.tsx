...
export default function NurseCertifications() {
  const { user } = useAuth();
  const [nurseFiles, setNurseFiles] = useState<NurseFile[]>([]);
  ...
  useEffect(() => {
    const loadFiles = async () => {
      ...
      try {
        ...
        url.searchParams.set('nurseEmail', user.email);
        const res = await fetch(url.toString());
        if (!res.ok) {
          throw new Error('Failed to load nurse files');
        }
        const data = (await res.json()) as NurseFile[];
        setNurseFiles(data);
      } catch (err: any) {
        setFilesError(err?.message || 'Failed to load your files.');
      } finally {
            ...
            <div className="healthcare-card">
              ...
              {isLoadingFiles ? (
                ...
                <p className="text-sm text-muted-foreground">
                  No files available yet. Your administrator can upload certification PDFs for you.
                </p>
              ) : (
