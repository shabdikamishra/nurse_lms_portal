...
export default function LiveClasses() {
  ...
  // Parse dates and create a map of classes by date
  ...
  // Get all dates with classes
  const datesWithClasses = new Set(
    upcomingClasses.map(c => new Date(c.date).toDateString())
  );

  // Custom modifier to highlight dates with classes
  const disabledDays = (date: Date) => {
    return !datesWithClasses.has(date.toDateString());
  };
      ...
      <div className="space-y-6 animate-fade-in">
        ...
        <Dialog open={showCalendarModal} onOpenChange={setShowCalendarModal}>
          <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>Classes Calendar</DialogTitle>
            </DialogHeader>
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2">
                {selectedDate ? (
                  <div>
                    ...
                    <div className="space-y-4">
                      {getClassesForDate(selectedDate).length > 0 ? (
                        getClassesForDate(selectedDate).map((liveClass) => (
                          <LiveClassCard key={liveClass.id} {...liveClass} />
                        ))
                      ) : (
                        <div className="text-center py-12">
                  ...
                  <div className="text-center py-12">
                    <Calendar className="w-12 h-12 text-muted-foreground mx-auto mb-4" />
                    <p className="text-muted-foreground">Select a date to view classes</p>
                  </div>
                )}
              </div>
            </div>
